...
var CanvasChart = function () {
    ...
    var renderBackground = function() {
        ...
        ctx.fillRect(margin.left, margin.top, xMax - margin.left+30, yMax - margin.top+20);
        ctx.fillStyle = 'white';
    };
    ...
    var renderText = function() {
        ...
        ctx.fillText(data.title, (chartWidth / 2), (margin.top / 2)+10);
        ...
        ctx.font = labelFont;
        ctx.fillText(data.yLabel, (yMax / 2) * -1, (margin.left / 4)+10);
        ctx.restore();
    };
    ...
    var renderLinesAndLabels = function () {
        ...
        for (var i = 0; i < data.dataPoints.length; i++) {
            ...
            ctx.fillText(txt, margin.left - ((txtSize.width >= 14) ? txtSize.width : 10) - 7, yPos +24);

            //x axis labels
            txt = data.dataPoints[i].x;
            txtSize = ctx.measureText(txt);
            ctx.fillText(txt, xPos+20, yMax + (margin.bottom / 3)+10);
            xPos += xInc;
        }
    ...
    var renderData = function(type) {
        ...
        for (var i = 0; i < data.dataPoints.length; i++) {
            ...
            if (type == renderType.points) {
                ...
                //Render circle
                ctx.arc(ptX+20, ptY+20, 8, 0, 2 * Math.PI, false)
                ctx.fill();
                ctx.lineWidth = 1;
                ctx.strokeStyle = '#000';
                ctx.stroke();
